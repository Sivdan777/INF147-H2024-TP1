...
double donnee_bnr;
unsigned int donnee_bcd1;
unsigned int donnee_bcd2;
unsigned int donnee_bcd3;
unsigned int donnee_bcd4;
...
void decoder_mot_a429 (unsigned int valeur, int* est_corrompu,
  ...
  *numero_mot_octal = (int)decimale_a_octale(numero_mot_decimal);
  ...
  } else if(*numero_mot_octal == NUMERO_MOT_UTC || *numero_mot_octal == NUMERO_MOT_ALTITUDE ||
            *numero_mot_octal == NUMERO_MOT_LATITUDE || *numero_mot_octal == NUMERO_MOT_LONGITUDE) {// Decoder en BNR
    *donnee_bnr = ((unsigned int) valeur & MASQUE_DONNEE_BNR) >> DECALAGE_DONNEE_BNR;
    *donnee_bnr *= RESOLUTION_MOT_LONGITUDE;
  }
